...
main(void)
{
  ...
  for(;;){
    ...
    if(pid < 0){
      printf(1, "init: fork failed\n");
  8c:	c7 44 24 04 21 08 00 	movl   $0x821,0x4(%esp)
  93:	00 
  ...
  a0:	e8 8d 02 00 00       	call   332 <exit>
    ...
    if(pid == 0){
      exec("sh", argv);
  a5:	c7 44 24 04 d4 0a 00 	movl   $0xad4,0x4(%esp)
  ac:	00 
  ad:	c7 04 24 34 08 00 00 	movl   $0x834,(%esp)
  b4:	e8 b1 02 00 00       	call   36a <exec>
      printf(1, "init: exec sh failed\n");
  b9:	c7 44 24 04 37 08 00 	movl   $0x837,0x4(%esp)
  c0:	00 
...
main(void)
{
  ...
  e1:	00 
  e2:	c7 04 24 06 08 00 00 	movl   $0x806,(%esp)
...
putc(int fd, char c)
{
 ...
 556:	e8 f7 fd ff ff       	call   352 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 573:	e8 da fd ff ff       	call   352 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 630:	e8 1d fd ff ff       	call   352 <write>
...
putc(int fd, char c)
{
 ...
 680:	e8 cd fc ff ff       	call   352 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 78a:	e8 2b fc ff ff       	call   3ba <sbrk>
  if(p == (char*)-1)
